...
def func2(y_truth, X_given, N_given, C, s, epsilon, K_test):
    f_predicted = []
    K_trick = gaussian_kernel(X_given, X_given, s)
    for k in range(1, K + 1):
        y_indexes = (y_truth == k) * 2 - 1
...
def predict(y_predicted_train, C, s):
    K_test = gaussian_kernel(X_test, X_train, s)
    y_predicted = func2(y_train, X_train, N_train, C, s, epsilon, K_test)
    y_pred = np.argmax(K_test, axis=1)
    max_val = np.max(K_test, axis=1)
...
y_predicted_test1 = predict(y_predicted_train1, C, s)
...
for c in C_values:
    ...
    # Test
    y_predicted_test = predict(y_predicted_train, c, s)
