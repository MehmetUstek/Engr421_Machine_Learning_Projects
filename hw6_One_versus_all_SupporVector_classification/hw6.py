...
def func(y_truth, X_given, N_given, C, s, epsilon):
    ...
    for k in range(1,K+1):
        ...
        yyK = np.matmul(y_indexes[:, None], y_indexes[None, :]) * K_train
        # print(yyK)
        P = cvx.matrix(yyK)
        ...
        w0 = np.mean(
            y_indexes[active_indices] * (1 - np.matmul(yyK[np.ix_(active_indices, support_indices)], alpha[support_indices])))
